...
Expected: Python ≥3.10 and pytest available. If pytest missing: `python3 -m pip install pytest`.
...
- [ ] **Step 6: gitignore the build output + commit**

Append `dist/` to `.gitignore` (build artifact, regenerated by CI):
```
dist/
```
```bash
git add deploy/build_stlite.py tests/test_build_stlite.py .gitignore
